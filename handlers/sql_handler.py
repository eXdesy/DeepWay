...
def register_user(telegram_id, username, password, status, verification, backup_code, token, donation_alerts_token, created_date):
    cursor.execute('''INSERT INTO `users` (telegram_id, current_telegram_id, username, password, status, verification, backup_code, token, donation_alerts_token, created_date) 
...
def create_subscription(telegram_id, media_id, datetime, expire_date, link, media_type):
    cursor.execute(f'''INSERT INTO `subscriptions` (buyer_id, media_id, subscription_date, expire_date, link, media_type) 
                    VALUES (%s, %s, %s, %s, %s, %s)''',
...
def count_user_subscriptions(telegram_id):
    cursor.execute('''SELECT u.username, COUNT(s.subscription_id) AS subscription_count FROM `users` u
                    LEFT JOIN `subscriptions` s ON u.current_telegram_id = s.buyer_id WHERE u.current_telegram_id = %s 
                    GROUP BY u.username''', (telegram_id,))
    result = cursor.fetchone()
    return result
...
def create_media(telegram_id, user_data, added_time, media_type):
    cursor.execute(f'''INSERT INTO `{media_type}` (owner_id, media_id, name, description, photo, category, subcategory, plan1_price, plan3_price, plan6_price, plan12_price, plan_price, verification, added_time) 
...
def create_media_report(reporter_telegram_id, username, media_id, report_description, media_type, created_date):
    cursor.execute('''INSERT INTO `reports` (reporter_telegram_id, username, media_id, report_description, media_type, report_status, created_date) 
